...
var turnWheel = {
    ...
    colors:[],					//转盘奖品区块对应背景颜色
    outsideRadius:280,			//转盘外圆的半径
    textRadius:200,				//转盘奖品位置距离圆心的距离
    insideRadius:108,			//转盘内圆的半径
    startAngle:0,				//开始角度
...
// 图片信息
var imgPb_5 = new Image();
imgPb_5.src = "../../../static/img/lottery/gift_02.png";
var imgPb_10 = new Image();
imgPb_10.src = "../../../static/img/lottery/gift_04.png";
var imgIphoneX = new Image();
imgIphoneX.src = "../../../static/img/lottery/gift_03.png";
var imgSorry = new Image();
imgSorry.src = "../../../static/img/lottery/2.png";
var imgHjsl = new Image();
imgHjsl.src = "../../../static/img/lottery/gift_01.png";
...
//    var canvas = ($("#wheelCanvas")).get()[0]; // 注意，jQuery获取的是包装过的对象，不是DOM对象,可以进行转换
    ...
    for(var index = 0 ; index < turnWheel.rewardNames.length ; index++)
    {
         ...
         * */
        var reg = new RegExp("||");
        var rewardName=rewardName.replace(reg,"");
        console.log(rewardName);
        ...
        //添加对应图标
        if(rewardName.indexOf("5拍币")>=0){
            // 注意，这里要等到img加载完成才能绘制
            imgPb_5.onload=function(){
                ctx.drawImage(imgPb_5,0,0);
            };
            ctx.drawImage(imgPb_5,0,0);
        }else if(rewardName.indexOf("10拍币")>=0){
            imgPb_10.onload=function(){
                ctx.drawImage(imgPb_10,0,0);
            };
            ctx.drawImage(imgPb_10,0,0);
        }else if(rewardName.indexOf("谢谢参与")>=0){
            imgSorry.onload=function(){
                ctx.drawImage(imgSorry,0,0);
            };
            ctx.drawImage(imgSorry,0,0);
        }else if(rewardName.indexOf("iPhoneX")>=0){
            imgIphoneX.onload=function(){
                ctx.drawImage(imgIphoneX,0,0);
            };
            ctx.drawImage(imgIphoneX,0,0);
        }else if(rewardName.indexOf("黄金手链")>=0){
            imgHjsl.onload=function(){
                ctx.drawImage(imgHjsl,0,0);
            };
            ctx.drawImage(imgHjsl,0,0);
        }
